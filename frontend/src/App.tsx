            ...
            <div className="p-6 sm:p-8">
                ...
                <div className="mb-8 border-b border-gray-700">
                    <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                        <button onClick={() => setActiveTab('video')} className={`flex items-center whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm transition-colors ${ activeTab === 'video' ? 'border-[#007BFF] text-[#007BFF]' : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-600' }`}>
                            <VideoIcon /> Video Analysis
                        </button>
                        <button onClick={() => setActiveTab('image')} className={`flex items-center whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm transition-colors ${ activeTab === 'image' ? 'border-[#E600FF] text-[#E600FF]' : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-600' }`}>
                            <ImageIcon /> Image Studio
                        </button>
                    </nav>
                </div>

                {activeTab === 'video' && <VideoWorkflowManager creditBalance={creditBalance ?? 0} isFetchingCredits={isFetchingCredits} updateCredits={updateCredits} getToken={getToken} />}
                {activeTab === 'image' && <ImageWorkflowManager creditBalance={creditBalance ?? 0} isFetchingCredits={isFetchingCredits} updateCredits={updateCredits} getToken={getToken} />}
            </div>
        </div>
    );
}
...
function ImageWorkflowManager({ creditBalance, isFetchingCredits, updateCredits, getToken }: WorkflowManagerProps) {
    ...
    const handleAnalysisComplete = useCallback((result: ImageAnalysisResult, newTopic: string, newDetails: string, preview: string) => {
        setAnalysisResult(result); setTopic(newTopic); setDetails(newDetails); setImagePreview(preview); setStep('analysis');
    }, []);

    const handleGenerationComplete = useCallback((result: ImageGenerationResult) => {
        setGeneratedResult(result); setStep('generation');
    }, []);

    const handleReset = useCallback(() => {
        setTopic(''); setDetails(''); setImagePreview(null); setAnalysisResult(null); setGeneratedResult(null); setStep('input');
    }, []);

    const renderStep = () => {
        switch (step) {
            case 'input': return <ImageInputForm onAnalysisComplete={handleAnalysisComplete} creditBalance={creditBalance} isFetchingCredits={isFetchingCredits} getToken={getToken} updateCredits={updateCredits} />;
            case 'analysis': return <ImageAnalysisDisplay analysis={analysisResult!} topic={topic} details={details} imagePreview={imagePreview!} onGenerationComplete={handleGenerationComplete} />;
            case 'generation': return <ImageGenerationOutput result={generatedResult!} onReset={handleReset} />;
        }
    };
    return <>{renderStep()}</>;
}
...
function ImageInputForm({ onAnalysisComplete, creditBalance, isFetchingCredits, getToken, updateCredits }: ImageInputFormProps) {
    ...
    const handleAnalyze = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch(`${BACKEND_API_URL}/api/analyze-image`, { method: 'POST', body: formData });
            if (!response.ok) { const errData = await response.json().catch(() => ({ error: 'Unknown server error' })); throw new Error(errData.error || 'Image analysis failed.'); }
            const data = await response.json();
            if (!data.analysis) { throw new Error("Received unexpected format from the analysis API."); }
            const token = await getToken({ template: 'supabase' });
            ...
            if (decrementError) { console.error("Credit decrement failed, but analysis succeeded.", decrementError); } 
            else { updateCredits(); }
...
function ImageAnalysisDisplay({ analysis, topic, details, imagePreview, onGenerationComplete }: ImageAnalysisDisplayProps) {
    ...
    const handleGenerate = async (e: React.FormEvent) => {
        ...
        setError('');
        setIsLoading(true);
        if (!BACKEND_API_URL) { setError("Configuration error."); setIsLoading(false); return; }
        try {
            const response = await fetch(`${BACKEND_API_URL}/api/generate-image-content`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ topic, details, analysis, intent, controlLevel, withTextOverlay, aspectRatio }),
            });
            if (!response.ok) { const errData = await response.json().catch(() => ({ error: 'Unknown server error' })); throw new Error(errData.error || 'Image generation failed.'); }
            const data = await response.json();
            if (!data.result) { throw new Error("Received unexpected format from the generation API."); }
    ...
    const sliderConfig = intent === 'AdaptRemix' ? { label: 'Creative Freedom', minLabel: 'Faithful Adaptation', maxLabel: 'Abstract Interpretation' } : { label: 'Style Adherence', minLabel: 'Loosely Inspired', maxLabel: 'Strictly Follow Style' };
    const getAspectRatioButtonClass = (ratio: AspectRatio) => `flex flex-col items-center gap-2 p-3 rounded-lg cursor-pointer transition-colors ${ aspectRatio === ratio ? 'bg-[#E600FF]/20 text-[#E600FF] border border-[#E600FF]' : 'bg-black/30 hover:bg-black/50 border border-transparent' }`;

    return (
        <div className="space-y-8">
            <h2 className="text-2xl font-bold text-brand-light text-center">Analysis Complete: Configure & Generate</h2>
            {error && (<div className="bg-red-500/20 border border-red-500/30 text-red-300 px-4 py-3 rounded-lg text-sm" role="alert"><strong className="font-bold">Error: </strong><span>{error}</span></div>)}
            <div className="flex flex-col md:flex-row gap-6 bg-black/30 p-6 rounded-lg border border-[rgba(255,255,255,0.1)]">
                ...
                <div className="md:w-3/4 space-y-4">
                    ...
                    <div><h4 className="font-semibold text-[#E600FF]">Detected Subjects:</h4><p className="text-sm text-[#F5F5F7]/80">{detectedSubjects}</p></div>
                    <div><h4 className="font-semibold text-[#E600FF]">Detected Style:</h4><p className="text-sm text-[#F5F5F7]/80">{detectedStyle}</p></div>
                </div>
            </div>
            <form onSubmit={handleGenerate} className="space-y-8">
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div onClick={() => setIntent('AdaptRemix')} className={`p-6 rounded-xl border-4 cursor-pointer transition-all ${ intent === 'AdaptRemix' ? 'border-[#E600FF] bg-black/50 shadow-[0_0_20px_0_rgba(230,0,255,0.2)]' : 'border-transparent bg-black/30 hover:bg-black/50' }`}>
                        ...
                        <div onClick={() => setIntent('ExtractStyle')} className={`p-6 rounded-xl border-4 cursor-pointer transition-all ${ intent === 'ExtractStyle' ? 'border-[#E600FF] bg-black/50 shadow-[0_0_20px_0_rgba(230,0,255,0.2)]' : 'border-transparent bg-black/30 hover:bg-black/50' }`}>
                ...
                <div>
                    ...
                    <div className="flex justify-center items-center gap-4">
                        <button type="button" onClick={() => setAspectRatio('1:1')} className={getAspectRatioButtonClass('1:1')}><Icon1x1 className="h-8 w-8" /><span className="text-xs font-semibold">1:1</span></button>
                        <button type="button" onClick={() => setAspectRatio('4:5')} className={getAspectRatioButtonClass('4:5')}><Icon4x5 className="h-8 w-8" /><span className="text-xs font-semibold">4:5</span></button>
                        <button type="button" onClick={() => setAspectRatio('9:16')} className={getAspectRatioButtonClass('9:16')}><Icon9x16 className="h-8 w-8" /><span className="text-xs font-semibold">9:16</span></button>
                    </div>
                </div>
                <div className="max-w-3xl mx-auto">
                    <label htmlFor="control-level" className="flex justify-between items-center text-lg font-medium text-white mb-4"><span>{sliderConfig.label}</span><span className="text-[#E600FF] font-semibold">{controlLevel}%</span></label>
                    <input id="control-level" type="range" min="0" max="100" value={controlLevel} onChange={(e) => setControlLevel(Number(e.target.value))} className="w-full h-2 bg-black/30 rounded-lg appearance-none cursor-pointer range-lg accent-[#E600FF]" disabled={isLoading}/>
                    <div className="flex justify-between text-sm text-gray-400 mt-2"><span>{sliderConfig.minLabel}</span><span>{sliderConfig.maxLabel}</span></div>
                </div>
...
function ImageGenerationOutput({ result, onReset }: ImageGenerationOutputProps) {
    ...
    const renderPost = (platform: string, content: string) => ( content ? (
        <div className="bg-black/40 p-4 rounded-lg border border-[rgba(255,255,255,0.1)]">
            <div className="flex justify-between items-center mb-3">
                <h4 className="font-semibold text-white">{platform}</h4>
                <button onClick={() => handleCopy(content, false, setCopyStatus, platform)} className="text-sm text-[#8A8A8E] hover:text-white transition-colors bg-white/10 p-2 rounded-md">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
                <div className="relative group">
                    ...
                    <button onClick={downloadImage} className="absolute top-2 right-2 bg-[#E600FF] text-white p-2 rounded-md opacity-0 group-hover:opacity-100 transition-opacity shadow-md hover:bg-[#b300c7]" title="Download Image">
...
function VideoWorkflowManager({ creditBalance, isFetchingCredits, updateCredits, getToken }: WorkflowManagerProps) {
    ...
    const handleAnalysisComplete = useCallback((result: AnalysisResult, newTopic: string) => {
        setAnalysisResult(result); setTopic(newTopic); setStep('analysis');
    }, []);
    const handleGenerationComplete = useCallback((content: string | string[], type: VideoOutputType) => {
        setGeneratedContent(content); setGeneratedType(type); setStep('generation');
    }, []);
    const handleReset = useCallback(() => {
        setTopic(''); setAnalysisResult(null); setGeneratedContent(null); setGeneratedType(null); setStep('input');
    }, []);

    const renderStep = () => {
        switch (step) {
            case 'input': return <VideoInputForm onAnalysisComplete={handleAnalysisComplete} creditBalance={creditBalance} isFetchingCredits={isFetchingCredits} getToken={getToken} updateCredits={updateCredits} />;
            case 'analysis': return <VideoAnalysisDisplay analysis={analysisResult!} topic={topic} onGenerationComplete={handleGenerationComplete} />;
            case 'generation': return <VideoGenerationOutput content={generatedContent!} type={generatedType!} onReset={handleReset} />;
        }
    };
    return <>{renderStep()}</>;
}
...
function VideoInputForm({ onAnalysisComplete, creditBalance, isFetchingCredits, getToken, updateCredits }: VideoInputFormProps) {
    ...
    const checkFileDuration = (file: File): Promise<void> => new Promise((resolve, reject) => {
        const video = document.createElement('video');
        video.preload = 'metadata';
        video.onloadedmetadata = () => { window.URL.revokeObjectURL(video.src); if (video.duration > MAX_DURATION_SECONDS) { reject(new Error(`Video is too long (${(video.duration / 60).toFixed(1)} mins). Max 2 mins.`)); } else { resolve(); }};
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) { setError(''); setVideoFile(null); setVideoSource(''); try { await checkFileDuration(file); setVideoFile(file); setVideoSource(file.name); } catch (err: any) { setError(err.message); } }
    };

    const handleDrop = useCallback(async (e: DragEvent<HTMLDivElement>) => {
        e.preventDefault(); e.stopPropagation(); setIsDragging(false);
        const file = e.dataTransfer.files?.[0];
        if (file && file.type.startsWith('video/')) { setError(''); setVideoFile(null); setVideoSource(''); try { await checkFileDuration(file); setVideoFile(file); setVideoSource(file.name); } catch (err: any) { setError(err.message); } } 
    ...
    const handleAnalyze = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            let finalFileUri = '', finalMimeType = '';
            if (videoFile) {
                setStatusMessage('Authorizing secure upload...');
                const authResponse = await fetch(`${BACKEND_API_URL}/api/create-signed-url`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileName: videoFile.name, contentType: videoFile.type }) });
                if (!authResponse.ok) throw new Error('Failed to get authorization.');
                const { signedUrl, path, token } = await authResponse.json();
                setStatusMessage('Uploading video...');
                const uploadResponse = await fetch(signedUrl, { method: 'PUT', headers: { 'Content-Type': videoFile.type, 'Authorization': `Bearer ${token}`, 'x-upsert': 'true' }, body: videoFile });
                if (!uploadResponse.ok) throw new Error('Failed to upload video.');
                setStatusMessage('Processing video (this may take time)...');
                const transferResponse = await fetch(`${BACKEND_API_URL}/api/transfer-to-gemini`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filePath: path, mimeType: videoFile.type }) });
                if (!transferResponse.ok) throw new Error(`Video transfer failed.`);
                const transferData = await transferResponse.json();
                finalFileUri = transferData.fileUri; finalMimeType = transferData.mimeType;
            } else {
                setStatusMessage('Checking video duration...');
                const durationResponse = await fetch(`${BACKEND_API_URL}/api/get-video-duration`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ videoUrl: videoSource }) });
                if (!durationResponse.ok) throw new Error('Could not validate video URL.');
                const { duration } = await durationResponse.json();
                if (duration > MAX_DURATION_SECONDS) { throw new Error(`Video is too long (${(duration / 60).toFixed(1)} mins). Max 2 mins.`); }
                finalFileUri = videoSource; finalMimeType = 'video/youtube';
            }
            setStatusMessage('Analyzing Viral DNA...');
            const response = await fetch(`${BACKEND_API_URL}/api/analyze`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ videoSource: finalFileUri, mimeType: finalMimeType }) });
        ...
        <form onSubmit={handleAnalyze} className="space-y-6">
            ...
            <div>
                ...
                <div onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave} className={`bg-black/30 border-2 border-dashed ${isDragging ? 'border-[#007BFF]' : 'border-brand-gray/40'} rounded-lg p-6 text-center cursor-pointer hover:border-[#007BFF] transition-colors group relative`}>
                    ...
                    <input id="video-link" type="text" value={videoSource} onChange={(e) => { setVideoSource(e.target.value); if (videoFile) setVideoFile(null); }} className="relative z-20 mt-4 w-full bg-brand-dark/50 border border-[rgba(255,255,255,0.1)] rounded-md px-4 py-2 text-brand-light placeholder-[#8A8A8E] focus:ring-2 focus:ring-[#007BFF] focus:outline-none" placeholder="https://youtube.com/watch?v=..." disabled={isLoading} />
                </div>
            </div>
            <div>
...
function VideoAnalysisDisplay({ analysis, topic, onGenerationComplete }: VideoAnalysisDisplayProps) {
    ...
    const handleGenerate = async (type: VideoOutputType) => {
        setIsLoading(true); setLoadingType(type); setError('');
        if (!BACKEND_API_URL) { setError("Configuration error."); setIsLoading(false); return; }
        try {
            const response = await fetch(`${BACKEND_API_URL}/api/generate-content`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ topic, outputType: type, analysis }) });
            if (!response.ok) { const errData = await response.json(); throw new Error(errData.error || 'Generation failed'); }
            const data = await response.json();
            if (!data.content) { throw new Error("Received empty content from generation API."); }
            onGenerationComplete(data.content, type);
        } catch (err: any) { setError(err.message); } 
        finally { setIsLoading(false); setLoadingType(null); }
    };

    const renderAnalysisSection = (title: string, content: React.ReactNode) => (
        <div className="bg-black/30 p-4 rounded-lg border border-[rgba(255,255,255,0.1)]"><h4 className="font-semibold text-[#007BFF] mb-2">{title}</h4><div className="text-sm text-[#F5F5F7]/80 space-y-1">{content}</div></div>
    );
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {renderAnalysisSection("Meta Data", <><p><strong>Style:</strong> {analysis.meta.analyzed_style}</p><p><strong>Tone:</strong> {analysis.meta.primary_tone}</p></>)}
                {renderAnalysisSection("Hook (First 3s)", <><p><strong>Technique:</strong> {analysis.hook_analysis.technique}</p><p><strong>Pacing:</strong> {analysis.hook_analysis.pacing}</p></>)}
                {renderAnalysisSection("Retention Signals", <><p><strong>Pacing:</strong> {analysis.retention_signals.pacing_strategy}</p><p><strong>Structure:</strong> {analysis.retention_signals.narrative_structure}</p></>)}
            ...
            <div className="pt-4 flex flex-col sm:flex-row gap-4">
                ...
                <button onClick={() => handleGenerate('AI Video Prompts')} disabled={isLoading} className="flex-1 px-6 py-3 font-bold text-white bg-gradient-to-r from-[#007BFF] to-[#E600FF] rounded-lg transition-opacity hover:opacity-90 shadow-lg hover:shadow-[0_0_20px_0_rgba(150,50,255,0.3)] disabled:opacity-50 flex items-center justify-center">
                    {isLoading && loadingType === 'AI Video Prompts' ? <Spinner /> : null} Generate VEO Prompts
                </button>
            </div>
        </div>
    );
                ...
                {isPromptArray ? (
                    <div className="space-y-4">
                        {content.map((prompt, index) => (
                            <div key={index} className="bg-black/40 p-3 rounded-md border border-white/10 flex justify-between items-start gap-3">
                                <textarea className="w-full bg-transparent text-sm text-brand-light/80 resize-y focus:outline-none" rows={3} value={prompt} readOnly />
                                <button onClick={() => handleCopy(prompt, false, setCopyStatus, `prompt-${index}`)} className="text-sm text-[#8A8A8E] hover:text-white transition-colors bg-white/10 p-2 rounded-md" title="Copy prompt">
                    ...
                    <div>
                        <div className="flex justify-end mb-4">
                            <button onClick={() => handleCopy(content, true, setCopyStatus, 'script')} className="flex items-center gap-2 text-sm text-[#8A8A8E] hover:text-white transition-colors bg-white/10 px-3 py-1 rounded-md">
